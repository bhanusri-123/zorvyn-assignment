...
const BOT_RESPONSES = {
  spend: "Based on April data, you've spent ₹32,228 so far. Top categories: Shopping (₹10,700), Food (₹4,770), and Transport (₹3,020).",
  money: "Breakdown: Income ₹1,05,000 | Expenses ₹43,228 | Savings ₹61,772. You're saving ~58.8% of income — excellent!",
  save: "Tips: 1) Reduce shopping (₹10.7K this month). 2) Cook at home — food delivery is high. 3) Set up auto-SIP for mutual funds.",
  budget: "Budget utilization: Food 47% ✅ | Shopping 89% ⚠️ | Entertainment 71% | Transport 63%.",
  default: "I can help with spending analysis, savings tips, and budget tracking. Try: 'How much did I spend?' or 'How can I save more?'",
};
...
export default function FinanceDashboard() {
  ...
  const NavLinks = ({ onItemClick }) => (
    <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto scrollbar-thin">
      {navItems.map(item => (
        <button key={item.id} onClick={() => { setActiveNav(item.id); onItemClick?.(); }}
  ...
  return (
    <div className="flex h-screen bg-gray-950 text-white overflow-hidden" style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
      <style>{`
        ...
        .card-hover:hover{transform:translateY(-2px);box-shadow:0 16px 32px rgba(0,0,0,.4);}
      ...
      {/* ── DESKTOP SIDEBAR ── */}
      <aside className={`${sidebarOpen ? "w-56" : "w-16"} hidden md:flex flex-col flex-shrink-0 bg-gray-900 border-r border-gray-800 transition-all duration-300 z-20`}>
        {/* Logo */}
        <div className="h-16 flex items-center px-4 border-b border-gray-800 gap-3 flex-shrink-0">
          ...
          {sidebarOpen && <span className="font-bold text-white tracking-tight text-lg">Zorvyn</span>}
        </div>
        {/* Role */}
        {sidebarOpen && (
      ...
      {mobileSidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-60 bg-gray-900 border-r border-gray-800 flex flex-col fade-in">
            ...
            <NavLinks onItemClick={() => setMobileSidebarOpen(false)} />
          </div>
        </div>
      )}
        ...
        <main className="flex-1 overflow-y-auto scrollbar-thin p-3 sm:p-4 lg:p-5">
          ...
          {activeNav === "dashboard" && (
            <div className="space-y-4">
              ...
              {/* Summary Cards — 2 col on mobile, 4 on xl */}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                {/* PIE CHART — matching reference exactly */}
                <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
                  ...
                  {loading ? <Shimmer h="h-48" rounded="rounded-xl" /> : (
                    <>
                      ...
                      {/* 2-col legend grid (matches reference image style) */}
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    ...
                    { icon: "🛍️", title: "Shopping Alert", text: `Shopping ${formatINR(catTotals["Shopping"] || 0)} is 89% of budget. Consider cutting back.`, color: "border-l-rose-500" },
